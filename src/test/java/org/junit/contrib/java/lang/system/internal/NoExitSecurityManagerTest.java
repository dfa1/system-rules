...
public class NoExitSecurityManagerTest {
	...
	@RunWith(Parameterized.class)
	public static class void_methods {
		@Parameters(name = "{0}")
		public static List<Object[]> data() {
			...
			for (Method method : NoExitSecurityManager.class.getMethods()) {
				if (!method.getDeclaringClass().equals(Object.class)
					...
					&& !method.getName().equals("checkExit"))
		...
		private void assertCallIsDelegated(
			...
			assertThat(invocation.getMethod().getReturnType()).isEqualTo(method.getReturnType());
			assertThat(invocation.getMethod().getName()).isEqualTo(method.getName());
			assertThat(invocation.getMethod().getParameterTypes()).isEqualTo(method.getParameterTypes());
